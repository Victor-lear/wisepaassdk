...
def __generateConfig():
    ...
    for i in range(9):
        deviceConfig = DeviceConfig(id = 'Floor'+str(i+1),
          ...
          retentionPolicyName = '')
        for j in range(20):
            analog = AnalogTagConfig(name = 'AP'+str(j+1) + 'x'+str(j+1),
        ...
        for j in range(20):
            analog = AnalogTagConfig(name = 'AP'+str(j+1) + 'y'+str(j+1),
            ...
            deviceConfig.analogTagList.append(analog)  
        for j in range(20):
            textTag = TextTagConfig(name = 'AP'+str(j+1) + 'name',
                description = 'AP name'+str(j+1),
                readOnly = False,
                arraySize = 0)
...
def __generateData():
      ...
      for i in range(9):
        ...
        for j in range(20):
            deviceId = 'Floor'+str(i+1)
            tagName = 'AP'+str(j+1) + 'name'
            value = str('LouisaCoffee')
            tag = EdgeTag(deviceId, tagName, value)
            edgeData.tagList.append(tag)
